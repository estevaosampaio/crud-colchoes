...
const Home = () => {
    ...
    //     setOpen(false);
    //     api.delete(`/excluir-colchao/${id}`).then(() => {
    //         // alert('Produto deletado!');
    //         setLoading(true);
    //         api.get('/listar-colchoes').then((response) => {
    //             setColchoes(response.data);
    //             setLoading(false);
    //         });
    //     });
    ...
    return (
        <div>
            ...
            {/* <Dialog key={colchao.id} fullScreen={fullScreen} open={open} onClose={handleClose} aria-labelledby="responsive-dialog-title">
                                ...
                                <Grid item key={colchao.id} xs={12} sm={6} md={4}>
                                    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                                        ...
                                        <CardContent sx={{ flexGrow: 1 }}>
                                            ...
                                            <Typography>
                                                Data de entrada: {parseInt(new Date(colchao.dataDeEntrada).getDate().toString()) + 1}/{parseInt(new Date(colchao.dataDeEntrada).getMonth().toString()) + 1}/
                                                {new Date(colchao.dataDeEntrada).getFullYear().toString()}
                                            </Typography>
